...
data_tg = thyro4 %>% dplyr::select(-c(Patient_ID, Visit_Number, Thyroglobulin_Level_Predicted, Visit_Timestamp))

x = data_tg %>% dplyr::select(-TgT) %>% data.frame() %>% as.matrix()
y = data_tg$TgT

cv_lasso <- cv.glmnet(x, y, alpha = 1) # alpha=1 for LASSO
plot(cv_lasso) # Shows MSE vs. log(lambda)

best_lambda <- cv_lasso$lambda.min
lasso_model <- glmnet(x, y, alpha = 1, lambda = best_lambda)
coef=(coef(lasso_model)[,1])
coefName = names(coef)
interestTgT = coefName[which(coef != 0)] 


```

``` {r, ct regressed on all covariates}
data_ct = thyro4 %>% dplyr::select(-c(Patient_ID, Calcitonin_Level_Predicted, Visit_Number, Visit_Timestamp)) 

x = data_ct %>% dplyr::select(-CtT) %>% data.frame() %>% as.matrix()
y = data_ct$CtT
...
coef=(coef(lasso_model)[,1])
coefName = names(coef)
interestCtT = coefName[which(coef != 0)]
...
data_rT3 = thyro4 %>% dplyr::select(-c(Patient_ID, Reverse_T3_Index, Visit_Number, Visit_Timestamp)) 
x = data_rT3 %>% dplyr::select(-rT3) %>% data.frame() %>% as.matrix()
y = data_rT3$rT3
...
coef=(coef(lasso_model)[,1])
coefName = names(coef)
interestrT3 = coefName[which(coef != 0)]


```

```{r}
D1 = data.frame(RV = c("Tg", "Ct", "rT3", "rT3", "rT3", "rT3"), 
                `Factors of Interest` = c(
                  paste0(interestTgT[-1], collapse = ", "),
                  paste0(interestCtT[-1], collapse = ", "),
                  paste0(interestrT3[-1][1:3], collapse = ", "),
                  paste0(interestrT3[-1][4:6], collapse = ", "),
                  paste0(interestrT3[-1][7:9], collapse = ", "),
                  paste0(interestrT3[-1][10:12], collapse = ", ")
                  )
                )
